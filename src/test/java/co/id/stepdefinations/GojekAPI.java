...
public class GojekAPI {
	...
	public void user_Execute_a_GET_API_request() throws Throwable {
		// response = GoJekFunctionUtility.executeApiRequest(strRequest);

	}

	@Then("^User validate Status Code$")
	public void user_validate_Status_Code() throws Throwable {
		// int intStatusCode = GoJekFunctionUtility.getstatusCode(response);
...
boolean bolTestStatus=false;
	...
	private String getAPIResponse(ArrayList arFile1Ap, int i) {
		...
		Response response = GoJekFunctionUtility.executeApiRequest(strFileApi);
		int intStatusCode = GoJekFunctionUtility.getstatusCode(response);
		String strResponseData = GoJekFunctionUtility.getResponseData(response);
